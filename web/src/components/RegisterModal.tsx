...
type Props = {
  open: boolean
  onClose: () => void
  onSuccess: (payload: RegisterResponse) => void
}
...
export default function RegisterModal({ open, onClose, onSuccess }: Props) {
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        const msg = (json as any)?.message || `Registration failed (${res.status})`
        throw new Error(msg)
      }
      const payload = json as RegisterResponse
      // Store tokens (adjust storage strategy if you prefer cookies/headers)
    ...
    >
      ...
      >
        <div className="flex items-start justify-between mb-4">
          <h2 id="register-title" className="text-xl font-semibold">Create your account</h2>
          <button
            type="button"
